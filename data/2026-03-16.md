...
Motivation: 解决如何将原始的AIS船舶轨迹数据转换为结构化、语义丰富的表示，使其既能被人理解又能被机器推理系统直接使用的问题。

Method: 提出上下文感知的轨迹抽象框架：1) 将噪声AIS序列分割为独立的航次；2) 每个航次进一步分为清洁的、带有移动性标注的片段；3) 用多源上下文信息（地理实体、离岸导航特征、天气条件等）丰富每个片段；4) 支持使用LLM生成受控的自然语言描述。

Result: 通过增加语义密度和降低时空复杂性，这种抽象表示可以促进下游分析，并支持与LLM集成进行更高级的海事推理任务。实验检验了多个LLM在AIS数据和开放上下文特征上生成描述的质量。

Conclusion: 该框架成功地将原始AIS轨迹数据转换为结构化、语义丰富的表示，支持LLM生成自然语言描述，为海事领域的下游分析和智能推理提供了有效的基础设施。
...
TL;DR: ReBalance是一个无需训练、即插即用的框架，通过基于置信度的动态控制来解决大型推理模型中的过度思考与思考不足问题，实现高效平衡推理。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型存在过度思考（在简单问题上浪费计算步骤）和思考不足（未能充分探索推理路径）的问题，导致效率低下和准确性受限。现有方法如抑制反思关键词或调整推理长度可能会无意中引发思考不足，损害准确性。

Method: ReBalance利用置信度作为推理动态的连续指标，通过高置信度方差识别过度思考，通过持续过度自信识别思考不足。将小规模数据集的隐藏状态聚合成推理模式原型，计算引导向量来指导模型推理轨迹。动态控制函数根据实时置信度调节该向量的强度和方向，在过度思考时剪枝冗余，在思考不足时促进探索。

Result: 在0.5B到32B的四个模型上，以及数学推理、通用问答和编码任务的九个基准测试中进行的广泛实验表明，ReBalance有效减少了输出冗余，同时提高了准确性。
...
TL;DR: 该论文将基于网页的AI智能体架构与传统规划算法对应，提出评估指标和数据集，比较不同架构的性能差异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在网页任务中像黑箱一样运行，难以诊断失败原因和规划过程，需要系统化的分析框架来理解不同智能体架构的行为特征。

Method: 1) 提出将现代智能体架构映射到传统规划范式的分类法：逐步执行对应广度优先搜索，树搜索对应最佳优先树搜索，预先完整规划对应深度优先搜索；2) 提出五个新的评估指标来评估轨迹质量；3) 创建包含794个人工标注轨迹的WebArena数据集；4) 比较逐步执行智能体与新颖的预先完整规划智能体。

Result: 逐步执行智能体更接近人类黄金轨迹（38%总体成功率），而预先完整规划智能体在技术指标如元素准确率（89%）上表现更优，表明需要根据具体应用约束选择合适的智能体架构。

Conclusion: 通过将智能体架构形式化为传统规划范式，可以系统诊断失败原因，提出的评估指标能更全面地评估智能体性能，为不同应用场景选择合适的智能体架构提供指导。
...
TL;DR: 提出基于偏差导数的早期检测方法，使用随机森林预测，通过数据增强解决训练数据不足问题，能提前预警船舶发动机灾难性故障


<details>
  <summary>Details</summary>
Motivation: 船舶发动机灾难性故障具有突发性和不可预测性，对航行、船员和乘客构成严重威胁。现有研究主要关注部件渐进退化，对突发异常现象关注有限，需要开发早期检测方法以防止不可逆损坏。

Method: 基于真实故障发动机数据，评估传感器实际读数与预期值之间偏差的导数。使用随机森林算法进行预测，这是测试中最合适的机器学习算法。传统方法关注监测信号的偏差，而本方法使用偏差的导数来提供异常动态的早期指示。采用深度学习数据增强解决训练数据获取问题。

Result: 仿真结果证实该方法能有效预测灾难性故障的发生。真实世界数据验证进一步强化了方法的鲁棒性和实际适用性。方法能在测量值达到临界阈值和触发警报之前检测异常，为操作员提供提前预警。

Conclusion: 提出的基于偏差导数的早期检测方法能提前预警船舶发动机灾难性故障，使操作员有时间安全关闭发动机、改变航线并避免潜在障碍，防止损坏和意外动力损失。
...
TL;DR: AIM是一种新的模型调制范式，让单个模型能展现多种行为以满足不同需求，通过logits重分配策略实现效用和焦点两种调制模式，无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 大规模模型通常需要适应不同的需求，但维护多个专门化版本效率低下。需要一种方法让单个模型能动态调整行为以满足模型所有者和用户的特定需求。

Method: 提出AIM模型调制范式，引入logits重分配策略，在训练数据无关且无需重新训练的情况下实现两种调制模式：效用调制（动态控制输出质量）和焦点调制（精确控制模型关注的输入特征）。基于联合概率分布建立统计理论基础。

Result: 在图像分类、语义分割和文本生成等任务上验证了AIM的实用性和多功能性，支持ResNet、SegFormer和Llama等多种主流架构。

Conclusion: AIM提供了一种高效灵活的模型调制方法，使单个模型能够适应多样化的需求，避免了维护多个专门化版本的开销，为AI模型调制提供了实用的解决方案。
...
### [6] [Context is all you need: Towards autonomous model-based process design using agentic AI in flowsheet simulations](https://arxiv.org/abs/2603.12813)
*Pascal Schäfer,Lukas J. Krinke,Martin Wlotzka,Norbert Asprion*

Main category: cs.AI

TL;DR: 本文提出了一个基于大语言模型的多智能体AI框架，用于化工流程模拟环境中的辅助建模，展示了在典型流程建模示例中的有效性。


<details>
  <summary>Details</summary>
Motivation: 虽然基于大语言模型的智能体AI系统在软件开发等领域已广泛应用，但在化工流程模拟领域仍处于探索阶段。本文旨在开发一个能够辅助工业流程模拟环境的AI框架。

Method: 首先展示了GitHub Copilot结合Claude Opus等先进LLM生成Chemasim工具有效语法的能力；然后开发了一个多智能体系统，其中一个智能体利用工程知识解决抽象问题，另一个智能体将解决方案实现为Chemasim代码。

Result: 该框架在典型流程建模示例中表现出有效性，包括反应/分离过程、变压精馏和包含夹带剂选择的异质共沸精馏等案例。

Conclusion: 本文提出的多智能体AI框架为化工流程模拟提供了有效的辅助工具，同时讨论了当前框架的局限性并展望了未来研究方向。
...
### [7] [ODRL Policy Comparison Through Normalisation](https://arxiv.org/abs/2603.12926)
*Jaime Osvaldo Salas,Paolo Pareti,George Konstantinidis*

Main category: cs.AI

TL;DR: 该论文针对ODRL语言复杂性导致的互操作性问题，提出了一种参数化规范化方法，将包含权限和禁止的复杂策略转换为仅包含权限的简化形式，并提供了保持语义的算法。
...
Method: 基于最近定义的语义，提出参数化规范化方法，将ODRL策略规范化为最小组件：将包含权限和禁止的策略重新表述为仅包含权限的策略，并将复杂逻辑约束简化为简单约束。提供了计算ODRL策略规范形式的算法，以及简化数值和符号约束的算法。

Result: 证明了这些算法能够保持策略的语义，并分析了结果的大小复杂度：在属性数量上呈指数级，在属性唯一值数量上呈线性。这使得复杂策略能够在ODRL的更基本片段中表示，并将策略比较问题简化为检查两条规则是否相同的更简单问题。

Conclusion: 提出的规范化方法解决了ODRL语言复杂性和互操作性问题，通过将策略转换为规范形式，简化了策略比较和处理，使复杂策略能够在更基本的ODRL片段中表示，提高了ODRL的实际可用性。
...
### [8] [Efficient and Interpretable Multi-Agent LLM Routing via Ant Colony Optimization](https://arxiv.org/abs/2603.12933)
...
TL;DR: AMRO-S是一个高效可解释的多智能体系统路由框架，通过语义条件路径选择、SFT小模型意图推断、任务特定信息素专家和质量门控异步更新机制，在保持低延迟的同时优化路由性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的多智能体系统面临高推理成本、延迟和透明度有限的问题，现有路由策略依赖昂贵的LLM选择器或静态策略，在动态负载和混合意图下缺乏可控性，导致性能不稳定和资源利用效率低下。

Method: 将MAS路由建模为语义条件路径选择问题：1) 使用监督微调的小语言模型进行意图推断，为每个查询提供低开销语义接口；2) 将路由内存分解为任务特定信息素专家，减少跨任务干扰；3) 采用质量门控异步更新机制，将推理与学习解耦。

Result: 在五个公共基准测试和高并发压力测试中，AMRO-S相比强基线路由方法持续改善了质量-成本权衡，同时通过结构化信息素模式提供可追溯的路由证据。

Conclusion: AMRO-S提供了一个高效且可解释的路由框架，解决了多智能体系统在动态负载和混合意图下的路由挑战，实现了更好的性能-成本权衡和可追溯性。
...
### [9] [Structured Distillation for Personalized Agent Memory: 11x Token Reduction with Retrieval Preservation](https://arxiv.org/abs/2603.13017)
*Sydney Lewis*

Main category: cs.AI

TL;DR: 研究提出个性化智能体记忆压缩方法，将长对话历史压缩为紧凑检索层，实现11倍压缩率，在保持96%检索性能的同时大幅降低存储成本。


<details>
  <summary>Details</summary>
Motivation: AI智能体与用户的长期对话历史虽然有用，但完整存储成本高昂，需要一种既能压缩存储又能保持检索效果的方法。

Method: 将每次对话交换压缩为包含四个字段的复合对象：exchange_core、specific_context、thematic room_assignments和regex-extracted files_touched。在4,182个对话（14,340次交换）上应用，平均将交换长度从371个token压缩到38个token。使用201个检索导向查询、107种配置和5个LLM评分器进行评估。

Result: 最佳纯压缩配置达到最佳原始文本MRR的96%（0.717 vs 0.745）。向量搜索配置无显著差异，而所有BM25配置显著退化。最佳跨层设置略优于最佳原始基线（MRR 0.759）。

Conclusion: 结构化蒸馏方法能在不牺牲检索质量的情况下压缩单用户智能体记忆，以1/11的上下文成本容纳数千次交换，同时保留原始文本用于深入查询。研究开源了实现和分析管道。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法主要关注最终答案准确性，忽视了推理过程的透明度和可验证性。需要开发能够评估中间推理步骤质量的基准，以揭示模型在推理链构建、步骤顺序等方面的系统性缺陷。

Method: 1) 构建CRYSTAL基准：通过Delphi式流程，由四个独立的MLLM生成推理轨迹，通过语义聚类聚合并经过人工质量验证；2) 提出两种评估指标：Match F1（基于语义相似度的步骤级精度和召回率）和Ordered Match F1（进一步惩罚无序推理链）；3) 提出因果过程奖励(CPR)：将答案正确性与步骤级对齐耦合的乘法奖励；4) 提出CPR-Curriculum：在训练中逐步增加推理难度的课程学习方法。

Result: 评估20个MLLM（包括未参与基准构建的前沿商业系统）发现：普遍存在"摘樱桃"现象（精度远高于召回率）、非单调缩放权衡、无序推理（没有竞争模型能保持超过60%的匹配步骤在正确顺序中）。CPR-Curriculum通过GRPO实现了+32%的Match F1提升，在加性奖励策略失败的情况下改善了推理能力。

Conclusion: CRYSTAL基准揭示了仅靠准确性无法发现的系统性推理缺陷。提出的因果过程奖励和课程学习方法能够有效提升模型的推理质量，为多模态推理模型的评估和改进提供了新方向。
...
TL;DR: Steve-Evolving：一个非参数自进化框架，通过细粒度执行诊断与双轨知识蒸馏的紧密耦合，在闭环中实现长期任务的经验组织与进化


<details>
  <summary>Details</summary>
Motivation: 开放世界具身智能体需要解决长期任务，主要瓶颈不是单步规划质量，而是如何组织和进化交互经验

Method: 采用三阶段框架：经验锚定（结构化经验元组+三层经验空间+多维索引）、经验蒸馏（成功轨迹提炼为可重用技能，失败提炼为可执行护栏）、知识驱动的闭环控制（检索技能和护栏注入LLM规划器，诊断触发的局部重规划在线更新活动约束）

Result: 在Minecraft MCU的长期任务套件上，相比静态检索基线实现了持续改进

Conclusion: Steve-Evolving框架通过紧密耦合执行诊断与知识蒸馏，实现了无需模型参数更新的持续进化过程，有效解决了长期任务中的经验组织问题
...
TL;DR: 本文提出了一种改进GRPO的方法，通过双边上下文条件化和奖励置信度校正机制，利用正确与错误推理轨迹之间的对比信息来提升推理模型的训练效果。


<details>
  <summary>Details</summary>
Motivation: GRPO虽然基于组均值计算优势函数，但在优化过程中将每个输出视为独立样本，忽略了组内正确与错误解决方案之间的自然对比结构。这种对比信息对于提升模型性能具有重要价值。

Method: 1. 提出对比性重构的GRPO，揭示其目标函数隐式地最大化正确与错误样本策略比率之间的边界；2. 提出双边上下文条件化机制，让模型在优化过程中交叉参考成功和失败的推理轨迹；3. 引入奖励置信度校正机制，利用奖励-置信度协方差动态调整GRPO中的优势基线。

Result: 在数学推理基准测试中，该方法在多种模型和算法上都表现出一致的性能提升，且无需额外采样或辅助模型。

Conclusion: 通过利用组内正确与错误推理轨迹之间的对比信息，提出的双边上下文条件化和奖励置信度校正机制能够有效提升GRPO的训练效果，为推理模型的优化提供了新的思路。
...
<details>
  <summary>Details</summary>
Motivation: 在资源匮乏地区，用户健康素养低且医疗资源有限，通过电话聊天机器人提供可信赖的孕产妇健康信息具有重要影响。但部署此类系统面临技术挑战：用户查询简短、不完整、跨语言混杂，答案需要地区特定背景，症状信息不全导致安全分流决策困难。

Method: 系统结合：(1) 阶段感知分流，将高风险查询路由到专家模板；(2) 对精选孕产妇/新生儿指南进行混合检索；(3) 基于LLM的证据条件生成。核心贡献是在有限专家监督下的高风险部署评估工作流程，包括组件级和端到端测试。

Result: 建立了四个评估基准：(i) 标记分流基准(N=150)实现86.7%的紧急情况召回率，明确报告漏检紧急情况与过度升级的权衡；(ii) 合成多证据检索基准(N=100)包含块级证据标签；(iii) 使用临床医生设计标准的LLM作为评判者对真实查询(N=781)进行比较；(iv) 专家验证。结果显示可信赖的医疗助手需要深度防御设计配合多方法评估。

Conclusion: 在多语言、嘈杂环境中构建可信赖的医疗助手需要深度防御设计配合多方法评估，而不是依赖单一模型和评估方法选择。系统通过学术研究者、健康科技公司、公共卫生非营利组织和医院的合作开发，展示了在资源匮乏环境中部署医疗AI的实际方法。
